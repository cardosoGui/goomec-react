...
function br({ active, onClick }) {
  function handler(where) {
    if (onClick) return e => onClick(e, where);
  }

  function isActive(where) {
    return where === active && "map-selected";
  }
    ...
    >
      <g>
        {states.map(state => (
          <a
            key={state.id}
            onClick={handler(state.id)}
            className={isActive(state.id)}
          >
            <title>
              {state.name}
              dasdsadas
